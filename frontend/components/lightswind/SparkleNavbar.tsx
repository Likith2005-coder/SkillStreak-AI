...
// Lightswind — Sparkle Navbar.
// Source: https://www.lightswind.com/components/sparkle-navbar
// Extended with `onSelect` + `initialIndex` so it can drive real navigation
// (the original only tracks active state on click). Header-friendly margin.

interface SparkleNavbarProps {
  /** Navigation labels, one button each. @example ['Home', 'About', 'Contact'] */
  items: string[];
  /** Accent color for the active strike/beam + glow. @example '#a855f7' */
  color?: string;
  /** Item to start on (e.g. derived from the current route). */
  initialIndex?: number;
  /** Fired when an item is chosen — use it to navigate. */
...
const SparkleNavbar: React.FC<SparkleNavbarProps> = ({
  ...
  // Keep the active indicator in sync with the route even when navigation
  // happens OUTSIDE the navbar (e.g. clicking a "Enter the Arsenal" button).
  // `initialIndex` is derived from the current pathname by the parent; -1
  // means "no item is current" (e.g. /settings) and hides the indicator.
  useEffect(() => {
    setActiveIndex(initialIndex);
    const activeButton = buttonRefs.current[initialIndex];
    if (activeElementRef.current && activeButton) {
  ...
  const handleClick = (index: number) => {
    ...
    if (index === activeIndex || !navElement || !activeElement || !newButton) {
      return;
    }

    onSelect?.(index, items[index]);

    // No previous active item (route outside the nav, activeIndex -1):
    // skip the strike animation and just show the indicator at the target.
    if (!oldButton) {
      gsap.set(activeElement, { x: getOffsetLeft(newButton) });
      gsap.to(activeElement, { "--active-element-show": "1", duration: 0.25 });
